...
// 2. 실수 : 소수점 있는 숫자, (부동소수점)실수는 정밀하게 계산이 불가능 하다. --> 별도의 정밀한 함수
console.log( 0.1 + 0.2)
...
// 2. 템플릿 리터럴 : '(백틱) 키보드 왼쪽[tab]키 위에, 변수나 식을 넣을때는 달러표시{ }
...
//[논리형] : true(참) , false(거짓) 의 값을 표현, 조건식에 주로 사용된다.
console.log(true)
console.log(false)
...
let 채소상자 = null;  // 변수 null이 대입/저장 된 상태 , 라이브러리 함수들이 null 반환하는 경우가 많다.(자료가 없다. 유효하지 않다.)
console.log( 채소상자 ) // 변수에 null 일 경우에는 , null

//2. 배열 호출 하는법
//(1) 배열내 모든 요소(값/자료) 들이 반환된다.
season = ['봄', '여름', '가을', '겨울']
...
console.log(season[4]) // underfined , 존재하지 않은 인덱스 이므로
// *[생각해보기1]
...
//6. 배열내 특정 요소 찾기, 변수명.indexof(찾을값), 찾을값이 배열내 존재하면 인덱스, 없으면 -1 반환
