...
3. 핵심 기능
    초기 데이터 표시:
        사용자가 페이지에 처음 방문했을 때, 
        카테고리 선택 메뉴에는 두 개의 예시 카테고리('음료', '과자')가 기본적으로 
        표시되어 있어야 합니다.
    ...
    제품 관리 기능:
        삭제: 각 제품의 "삭제" 버튼을 클릭하면 해당 제품이 목록에서 
        즉시 제거되어야 합니다.
        수정: "수정" 버튼을 클릭하면, prompt 창을 통해 
        새로운 제품명과 가격을 입력받아 해당 제품의 정보를 수정하고 
        목록을 즉시 갱신해야 합니다.
*/
/*
    [작업순서]
    기획자 : 생각하는코드 vs 개발자 : 생각하는코드
    1.[프] 화면 스케치(프로토타입)
    2.[프] HTML과 CSS표현
    3.[백] 데이터 모델링
    4.[백] 기능(함수)설계
...
let currentCno = 2; //*코드를 자동대입 하기 위한 
// 현재 코드번호, 샘플의 마지막 코드의 번호 초기화
// 2. 제품 목록
const productList = [{ pno: 1, cno: 1, pname: '코카콜라', pprice: 1000, pimg: 'https://placehold.co/100x100', pdate: '2025-06-17' },
{ pno: 2, cno: 2, pname: '새우깡', pprice: 1200, 
    pimg: 'https://placehold.co/100x100', pdate: '2025-06-18' },
{ pno: 3, cno:1, pname: '칠성사이다', pprice: 900,
     pimg: 'https://placehold.co/100x100', pdate: '2025-6-19' },
];
let currentPno = 3;
console.log(categoryList);
console.log(productList);
//=======================[2] 데이터 모델링 샘플======================//
//1. 카테고리 출력함수 : 카테고리목록의 객체정보를 <select>에 출력하는 함수
//1. 실행조건 : 페이지가 JS가 열렸을때 실행
categoryPint();
function categoryPint() {
    ...
    for (let index = 0; index <= categoryList.length - 1; index++) {  //index번째 카테고리 1개 객체 꺼내기
        const category = categoryList[index]; //index번째 카테고리(1개)객체 꺼내기
...
//3. 제품목록 출력함수 :
//3. 실행조건 : (1) 페이지 HTML/JS 페이지가 열렸을때 함수실행, (2)등록/삭제/수정 완료 했을때
productPrint();
...
function productEdit( pno ){console.log('>>productEdit exe'); 
    for(let index=0; index <= productList.length-1; index++){
        if(productList[index].pno == pno){
