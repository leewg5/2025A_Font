...
[DOM객체]
    ...
    2. 내장객체 : document
    3. 내장객체내 주요 속성
        1. document.write('출력할 내용') : HTML에 문자열(HTML) 출력하는 함수
        2. document.querySelector('선택자') : HTML에 지정한 선택자를 JS객체로 반환하는 함수
        *선택자 : 1.마크업명  2. .클래스명  3. #id명
        3. document.querySelectorAll('선택자') : HTML에 지정한 선택자 모두 js객체(배열)로 반환하는 함수

    4. dom객체.innerHTML  : 마크업내 HTML 반환하는 속성
            <마크업> innerHTML  </마크업>  : 마크업과 마크업 사이가 innerHTML 이다.
            *inner없음 : <input /> , <img />
    5. dom객체.value 
        *<마크업 value=" "/>
        *value있음 : input, select , textarea, radio
...
//[6]
// 함수 실행 조건 : HTML에 있는 함수1실행 버튼을 클릭했을때
// 매개변수 : x , 반환값 : x , 
// 로직 : 1콘솔출력
function func1() {
    console.log('----------func1 exe---------'); //1.콘솔출력
