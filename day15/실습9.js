...
/* 작업순서
    ...
    5. (로직)
        1. 현재 회원들 중에서 구독중인 회원찾기
            ...
            (3) 일치한 회원코드가 있으면(구독중인 상태) //정보가 없으면(구독상태가 아니면)'구독내역이 없습니다' 를 document.write로 출력하시오.
...
// const today ="2025-06-17";
for(let i = 0; i <= 회원테이블.length - 1; i++) {  // i는 0부터 마지막 인덱스(회원)까지 1씩증가
    let 회원 = 회원테이블[i];   //i번째 회원(객체) 조회
    let 구독상태 = false; // 구독중이다(true)/구독중이 아니다(false)
    let 구독활성화 ='';
    for (let j = 0; j <= 구독로그테이블.length - 1; j++) {
        ...
        if (회원.회원코드 == 구독로그.회원코드) {
            구독상태 = true; // 현재 회원코드가 구독로그에 존재한다.(체크!!)
            구독상품코드 = 구독로그.상품코드; // i번째 회원의 구독중인 상품코드를 기록한다.
    ...
    if (구독상태 == true) {
        ...
        for(let index =0; index <= 구독상품테이블.length-1; index++){
            ...
            if(상품정보.상품코드 == 구독상품코드){ //index번째 구독상품코드와 i번째 회원의 구독중인 상품코드와 같으면
                html += `<ul><li>구독상품 : ${상품정보.상품명}(${상품정보.가격})</li>
                        <li>상태: ${구독활성화}</li></ul>`
            }           
        }


        html += `<ol><li>구독상품:</li></ol>`
    } else {
        html += `<div> 구독 내역이 없습니다.`
    }   //{document.write('구독중');}  else //{ document.write('구독 내역이 없습니다.')}
    document.write(html);
    document.write('<hr/>')
}// for1 end
