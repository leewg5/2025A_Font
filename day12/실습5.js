...
let fruitList = [ '사과', '바나나' ];   // 조건: 배열찾기함수(.indexOf()) , fruitList.indexOf(새로운 과일명) == -1
...
let coin = Number(prompt('총 구매금액:'))
if(coin >= 50000 * 10%){console.log('최종금액은:')}
else if((coin >= 30000 || coin <= 50000 )*5%){console.log('최종금액은:')}
else if((coin >= 10000 || coin <= 30000 )*1%){console.log('최종금액은:')}
else{console.log('최종금액은:')}
...
/*   A=10;  B=20; 두 변수간의 자료를 교체하시오.  A=B; B의 자료를 A에 대입 *A의 기존 자료는 사라진다.(변수는 하나의 자료로 저장한다.)  */
/* A=20;   B=20;
...
//(4) 오름차순: 두수 간의 작은 값을 앞에 나열, 두 수 비교하기.
// A > B 비교하여 A가 더크면 A와 B의 위치를 변경/스왑
//(5) 조건: num1 > num2, num1 > num3, num2 > num3

let temp;  //임시변수!!       부호 = [ > 오름차순, < 내림차순 ]  //시계방향으로 스왑
if(numList[0] > numList[1]){   //첫번째 값이 두번째 값보다 크면 (스왑/교체)
    temp = numList[0];          //임시변수에 첫번째 값을 넣고
    numList[0] = numList[1];    // 첫번째 위치에 두번쨰 값을 넣고
    numList[1] = temp;          // 두번째 위치에 임시변수 (첫번째값) 값을 넣고
} // 첫번째 값이 두번쨰 값보다 크면 (스왑/교체)
...
//(방법2)
//1. 두수가 같으면 무승부이다.  0 == (2+1)%3, 1 == (0+1)%3 , 2 == (1+1)%3
//2. 플레이어2 값의 + 1 하고 나누기 3했을때 나머지가 플레이어1과 같으면 플레이어1 승리
...
locationArray에 있는 그 차량의 주차 위치를 찾아 출력하는 프로그램을 작성하시오. 만약 해당하는 차량 번호가 없다면 
'차량이 존재하지 않습니다.'를 출력하시오.
